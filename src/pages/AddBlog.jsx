...
const AddBlog = () => {
  ...
  const categories = ["Technology", "Travel", "Lifestyle", "Food", "Business", "Health"];
  ...
  const handleImageUpload = (event) => {
    ...
    handleInputChange('images', files.map(file => URL.createObjectURL(file)));
  };

  const handleSave = () => {
    const requiredFields = ['title', 'author', 'keyword', 'category', 'readTime', 'summary'];
        ...
        {formData.content.map((c, index) => (
          <div key={index} className="flex gap-2 items-start mt-4">
            ...
            {index === formData.content.length - 1 && (
              <button 
                onClick={() => handleInputChange('content', [...formData.content, ""])}
